...
import { Typography, Box, CircularProgress, Chip, Button, Link } from "@mui/material";
...
const RewardsTab: React.FC<RewardsTabProps> = ({ }): JSX.Element => {
    ...
    const handleClaimRewards = async (restake: boolean = false) => {
        try {
            ...
            if (rewardsInfo.status !== "success") { return }
            var indexes = rewardsInfo.unclaimedIntervals.map((reward) => reward.index).join(",");
            var canClainRewards = restake ? 
                await appService.getNodeCanClaimAndRestakeRewards(indexes, toWei(nodeRewards?.unclaimedRplRewards ?? 0)) : 
                await appService.getNodeCanClaimRewards(indexes);
            if (canClainRewards.status !== "success") { return }
            var claimRewards = restake ? 
                await appService.nodeClaimAndRestakeRewards(indexes, toWei(nodeRewards?.unclaimedRplRewards ?? 0)) : 
                await appService.nodeClaimRewards(indexes);
            setTxs([...txs, claimRewards.txHash])
            if (claimRewards.status !== "success") { return }
    ...
    return (
        <Box>
            <Typography variant="h5">Claim Rewards (could take a few minutes):</Typography>
            ...
            {!isLoading && nodeRewards && (
                <>
                    ...
                    <Box sx={{ marginTop: 3 }}>
                        <Button
                            disabled={isClaiming || (((nodeRewards?.unclaimedEthRewards ?? 0) === 0) && ((nodeRewards?.unclaimedRplRewards ?? 0) === 0))}
                            variant="contained"
                            onClick={() => handleClaimRewards()}
                        >
                            {" "}
                            {isClaiming ? <CircularProgress size={24} color="primary" /> : `Claim`}
                        </Button>
                        <Typography variant="subtitle1">
                            or
                        </Typography>
                        <Button
                            disabled={isClaiming || ((nodeRewards?.unclaimedRplRewards ?? 0) === 0)}
                            variant="contained"
                            onClick={() => handleClaimRewards(true)}
                        >
                            {" "}
                            {isClaiming ? <CircularProgress size={24} color="primary" /> : `Claim & Restake RPL tokens`}
                        </Button>
                    </Box>
                    {txs.map((tx, index) => (
